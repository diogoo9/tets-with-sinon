...
describe("File Management", () => {
    ...
    it("Should a new file ", ()=>{
        const writeStub = sinon.stub(fs, "writeFileSync");
        const fileManagement = proxyquire("./file.management", { fs});

        fileManagement.createFile("teste.txt");
    ...
    it("Should throw an exception if file  already exists", ()=>{
        ...
        expect(()=>fileManagement.createFile("teste.txt")).to.throw();
    });
    ...
    it("getAllFiles should return a list of files",()=>{
        ...
        fileManagement.getAllFiles((err,data)=>{
            expect(data).to.eql(["test.txt"]," string not equal");
        });
    });

    it("getAllFilesPromisses",()=>{
        const readStub = sinon.stub(fs, "readdir");
        const util = { promisify: sinon.stub().returns(readStub)};
